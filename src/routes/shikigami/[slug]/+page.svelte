...
<script>
  ...
  // shikigami stats
  const mvsc_count = curr_shiki_obj.式神基础属性.移动速度
    .toString()
    .split("").length;
  let base_movement_speed;
  mvsc_count >= 3
    ? (base_movement_speed =
        curr_shiki_obj.式神基础属性.移动速度.toString().slice(0, 2) +
        curr_shiki_obj.式神基础属性.移动速度.toString().slice(1, 2))
    : (base_movement_speed = `${curr_shiki_obj.式神基础属性.移动速度.toString()}0`);

  const atk_speed =
    curr_shiki_obj.式神基础属性.攻击速度 *
    (1 + curr_shiki_obj.式神基础属性.攻速加成);
  const base_atk_speed = atk_speed.toFixed(2);

  const _growth_atk_speed =
    curr_shiki_obj.式神基础属性.攻速加成 + curr_shiki_obj.式神属性成长.攻速加成;
  ...
  function getDamageType() {
    ...
    return damage_types.filter((damage) =>
      skill_descriptions.toLowerCase().includes(damage)
    );
  }
  ...
  try {
    ...
    console.log(
      "[cybrejon] could not load Disqus, this is expected during development"
    );
  }
</script>
...
<div class="container">
  ...
  >
    ...
    {#if getDamageType().includes("physical damage")}
      <DamageTypePill damage_type="physical">Physical</DamageTypePill>
    {/if}
    {#if getDamageType().includes("magic damage")}
      <DamageTypePill damage_type="magic">Magic</DamageTypePill>
    {/if}
    {#if getDamageType().includes("true damage")}
      <DamageTypePill damage_type="true">True</DamageTypePill>
    {/if}
  </Container>
    ...
    {#if basicPage === 1}
      <Basic
        ...
        data={{
          ...
          classification: curr_shiki_obj.式神定位.map(
            (role) => dictionary.roles[role]
          ),
            ...
            <div class="bio">
              ...
              <div class="bio-contributor-container">
                {#if bioData.bio1 === "no bio yet"}
                  <p class="bio-contributor">
                    Join <a href="http://discord.gg/KGsaAet">my discord</a> to submit
                    missing bios!
                  </p>
                {:else}
                  <p class="bio-contributor">
                    Submitter: {bioData.contributor}
                  </p>
                  <p class="bio-contributor">
                    Source: <a
                      href="https://onmyoji.fandom.com/wiki/Onmyoji_Wiki"
                      >onmyoji.fandom.com</a
                    >
                  </p>
                {/if}
              </div>
    ...
    {#if is_order_visible}
      <div class="skill-order">
        <table>
          <thead>
            <tr>
              <td class="tr-skill-image-cell"
                ><img
                  loading="lazy"
                  src={images[curr_shiki_obj.式神技能.一技能.图标路径]}
                  alt="skill"
                /></td
              >
          ...
          <tbody>
            <tr>
              <td class="tr-skill-image-cell"
                ><img
                  loading="lazy"
                  src={images[curr_shiki_obj.式神技能.二技能.图标路径]}
                  alt="skill"
                /></td
              >
              {#each skill_order_data as order, i}
            ...
            <tr>
              <td class="tr-skill-image-cell"
                ><img
                  loading="lazy"
                  src={images[curr_shiki_obj.式神技能.三技能.图标路径]}
                  alt="skill"
                /></td
              >
              {#each skill_order_data as order, i}
            ...
            <tr>
              <td class="tr-skill-image-cell"
                ><img
                  loading="lazy"
                  src={images[curr_shiki_obj.式神技能.四技能.图标路径]}
                  alt="skill"
                /></td
              >
              {#each skill_order_data as order, i}
