...
export async function GET({ url }) {
  ...
  const itemTier = url.searchParams.get('item_tier');
  ...
  async function getItemData() {
    ...
    itemNames.forEach(name => {
      itemData.push({
        ...
        special_description: data.data[name].装备额外描述 == null ? undefined : data.data[name].装备额外描述,
        type: data.data[name].装备类型,
        composition: data.data[name].子物品,
        skills: {
          passive: data.data[name].装备被动技能 == null ? undefined : data.data[name].装备被动技能,
          active: data.data[name].装备主动技能 == null ? undefined : data.data[name].装备主动技能
        },
